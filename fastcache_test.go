...
func TestCacheSmall(t *testing.T) {
	...
	if v := c.Get(nil, 1); len(v) != 0 {
		t.Fatalf("unexpected non-empty value obtained from small cache: %q", v)
	}
	if v, exist := c.HasGet(nil, 1); exist || len(v) != 0 {
		t.Fatalf("unexpected non-empty value obtained from small cache: %q", v)
	}

	c.Set(2, []byte("value"))
	if v := c.Get(nil, 2); string(v) != "value" {
		t.Fatalf("unexpected value obtained; got %q; want %q", v, "value")
	}
	if v := c.Get(nil, 0); len(v) != 0 {
		t.Fatalf("unexpected non-empty value obtained from small cache: %q", v)
	}
	if v, exist := c.HasGet(nil, 0); exist {
	...
	c.Set(1, []byte("bbb"))
	if v := c.Get(nil, 1); string(v) != "bbb" {
		t.Fatalf("unexpected value obtained; got %q; want %q", v, "bbb")
	}
	if v, exist := c.HasGet(nil, 1); !exist || string(v) != "bbb" {
		t.Fatalf("unexpected value obtained; got %q; want %q", v, "bbb")
	}
...
func TestCacheWrap(t *testing.T) {
	...
	for i := uint64(0); i < calls; i++ {
		...
		if string(vv) != string(v) {
			t.Fatalf("unexpected value for key %d; got %q; want %q", k, vv, v)
		}
	}
